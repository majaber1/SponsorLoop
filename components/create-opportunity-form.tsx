...
export function CreateOpportunityForm({ locale }: { locale: Locale }) {
  ...
  return <form className="form-card panel" onSubmit={submit}><div className="form-grid">
    ...
    <label className="field"><span>{ar?"الفئة":"Category"}</span><select value={form.category} onChange={e=>setForm({...form,category:e.target.value as OpportunityCategory})}><option value="events">Events</option><option value="creators">Creators</option><option value="podcasts">Podcasts</option><option value="sports">Sports</option><option value="athletes">Athletes</option><option value="hackathons">Hackathons</option><option value="clubs">Clubs</option><option value="digital">Digital</option><option value="ooh">OOH</option><option value="community">Community</option><option value="gaming">Gaming</option></select></label>
    <label className="field"><span>{ar?"المدينة":"City"}</span><select value={form.city} onChange={e=>setForm({...form,city:e.target.value})}><option>Riyadh</option><option>Jeddah</option><option>Dammam</option><option>Makkah</option><option>Madinah</option><option>Khobar</option><option>Tabuk</option></select></label>
